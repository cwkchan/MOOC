...
echo "INFO: Loading database $dbname"  
        mysql -u "$user" -p"$password"  -e "drop database if exists \`$dbname\` ; create database \`$dbname\` " 

        echo "INFO: Uploading the following files..."
        find "$homeDir" -name "*$dbname*.sql" | awk '{ print "source",$0 }'
        find "$homeDir" -name "*$dbname*.sql" | awk '{ print "source",$0 }' | mysql -u "$user" -p"$password" "$dbname"
else 
while read database; do
	...
	echo "INFO: Uploading the following files..."
	find "$homeDir" -name "*$database*.sql" | awk '{ print "source",$0 }'
	#find "$homeDir" -name "*$database*.sql" | awk '{ print "source",$0 }' | mysql -u "$user" -p"$password" "$database"   
