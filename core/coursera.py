from . import Base
from util.config import *
import os.path as path

from sqlalchemy import Column, Integer, String, DateTime, Boolean


class Course(Base):
    __tablename__ = "coursera_courses"

    session_id = Column(String(255), index=True, unique=True)
    admin_id = Column(Integer, unique=True, primary_key=True)
    course = Column(String(255))
    instructor = Column(String(255), nullable=True)
    start = Column(DateTime, nullable=True)
    end = Column(DateTime, nullable=True)
    duration = Column(Integer, nullable=True)
    enrollment = Column(String(255), nullable=True)
    url = Column(String(1024), nullable=True)
    allow_signature = Column(Boolean, nullable=True)

    def __repr__(self):
        return "Course {} has PK {} and an admin_id of {}, is called {} and starts on {}.".format(self.session_id,
                                                                                                  self.id,
                                                                                                  self.admin_id,
                                                                                                  self.course,
                                                                                                  str(self.start))

    def has_clickstream(self):
        """Determines whether the clickstream files for this course have already been downloaded.

        :return: True if a clickstream file exists in the position get_properties()['clickstream'], False if it doesn't
        """
        return path.isfile("{}/{}_clickstream_export.gz".format(get_properties()['clickstream'], self.session_id))

    def has_sql(self):
        """Determines whether the sql files for this course have already been downloaded.

        :return: True if *all* of the sql files exist for this course, False if at least one doesn't
        """
        return path.isfile(
            "{}/{} ({})_SQL_anonymized_forum.sql".format(get_properties()['sql'], self.course,
                                                         self.session_id)) and path.isfile(
            "{}/{} ({})_SQL_anonymized_general.sql".format(get_properties()['sql'], self.course,
                                                           self.session_id)) and path.isfile(
            "{}/{} ({})_SQL_hash_mapping.sql".format(get_properties()['sql'], self.course,
                                                     self.session_id)) and path.isfile(
            "{}/{} ({})_SQL_unanonymizable.sql".format(get_properties()['sql'], self.course, self.session_id))

    def has_intent(self):
        """Determines whether the intent files for this course have already been downloaded.

        :return: True if an intent file exists in the position get_properties()['clickstream'], False if it doesn't
        """
        return path.isfile("{}/{}.csv".format(get_properties()['intent'], self.session_id))

    def has_demographics(self):
        """Determines whether the demographics files for this course have already been downloaded.

        :return: True if an intent file exists in the position get_properties()['clickstream'], False if it doesn't
        """
        return path.isfile("{}/{}.csv".format(get_properties()['demographics'], self.session_id))

